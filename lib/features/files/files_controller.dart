...
class FilesController {
  ...
  // scan today's dd.mm.yyyy/session_N; return max session index for current day
  Future<int> getMaxExistingSessionNumber() async {
    final root = await recordingsDirectory;
    if (!await root.exists()) return 0;

    final todayFolder = DateTime.now().format('dd.MM.yyyy');
    final todayDir = Directory(joinPath(root.path, todayFolder));
    if (!await todayDir.exists()) return 0;

    int maxSession = 0;
    await for (final sub in todayDir.list(recursive: false, followLinks: false)) {
  ...
  // new session_N folder per call; date subfolder; numbering resets per day
  Future<SessionPath> getNextSessionPath() async {
    ...
    // Find max existing session number in today's date folder
    final dateDir = Directory(dateDirPath);
    int maxInDay = 0;
    if (await dateDir.exists()) {
      await for (final entity
          in dateDir.list(recursive: false, followLinks: false)) {
        if (entity is! Directory) continue;
        final name = entity.path.split(Platform.pathSeparator).last;
        final match = sessionFolderPattern.firstMatch(name);
        if (match != null) {
          final n = int.tryParse(match.group(1) ?? '0') ?? 0;
    ...
    final sessionNumber = maxInDay + 1;
    await settingsController.setLastSessionNumber(sessionNumber);
    ...
    final ext = settingsController.effectiveFileExtension;
    final hz = settingsController.recordingSamplingRateHz.value;
    final filename = 'session_${sessionNumber}_${hz}hz$ext';
