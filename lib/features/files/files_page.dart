...
class FilesPageState extends State<FilesPage> {
    ...
    return Container(
      ...
      child: ListTile(
        ...
        title: Text(info.name,
            style: const TextStyle(color: AppTheme.textPrimary)),
        subtitle: Text(
          'Дата: ${info.formattedModified}   •   Размер: ${info.formattedSize}   •   ${info.duration.format()}',
  ...
  Future<void> showPolysomnographyUploadDialog(
    ...
    if (errors.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Загружено файлов: $uploaded')),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            uploaded > 0
                ? 'Загружено: $uploaded, ошибки: ${errors.join('; ')}'
                : 'Ошибка отправки: ${errors.join('; ')}',
          ),
