...
class PolysomnographyApiService {
  ...
  // extracts sampling rate from filename pattern _(\d+)hz; returns null if not found
  static int? parseSamplingRateFromFilename(String filename) {
    final match = RegExp(r'_(\d+)hz', caseSensitive: false).firstMatch(filename);
    return match != null ? int.tryParse(match.group(1)!) : null;
  }

  // linear interpolation resampling; no-op if rates are equal
  static List<double> resampleSamples(
      List<double> samples, int fromHz, int toHz) {
    if (fromHz == toHz) return samples;
    final ratio = fromHz / toHz;
    final count = (samples.length / ratio).floor();
  ...
  // strips # headers; resamples from sourceHz to targetHz if they differ
  Future<List<int>> _prepareFileBytes(File file,
      {required int sourceHz, required int targetHz}) async {
    final content = await file.readAsString();
    final lines = content
        .split('\n')
        .where((l) => !l.trimLeft().startsWith('#') && l.trim().isNotEmpty)
        .toList();
    if (sourceHz == targetHz) return lines.join('\n').codeUnits;
    final samples =
        lines.map((l) => double.tryParse(l.trim()) ?? 0.0).toList();
    final resampled = resampleSamples(samples, sourceHz, targetHz);
    return resampled.map((v) => v.toStringAsFixed(6)).join('\n').codeUnits;
  }

  // multipart upload; edf skips sampling_frequency param
  // for txt files: resamples from filename hz to polysomnographySamplingRateHz (100)
  Future<Map<String, dynamic>> uploadPatientFile({
    ...
    final sourceHz = parseSamplingRateFromFilename(filename) ??
        PolysomnographyConstants.defaultSamplingFrequencyHz.toInt();
    final targetHz = isEdf ? sourceHz : RecordingConstants.polysomnographySamplingRateHz;

    final queryParams = buildUploadQueryParams(
      patientId: patientId,
      patientName: patientName,
      isEdf: isEdf,
      samplingFrequency: isEdf ? null : targetHz.toDouble(),
    );

    final fileBytes = await _prepareFileBytes(file,
        sourceHz: sourceHz, targetHz: targetHz);
